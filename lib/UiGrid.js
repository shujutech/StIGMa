...
UiGrid.NotInList = function(aSelectedList, aSelectedObj) {
	return(UiGrid.ExistInList(aSelectedList, aSelectedObj) === undefined);
};
UiGrid.ExistInList = function(aSelectedList, aSelectedObj) {
	var result = undefined;
	for(var cntr = 0; cntr < aSelectedList.length; cntr++) { 
		var strObj = JSON.stringify(aSelectedList[cntr]);
		if (strObj === aSelectedObj) {
			result = cntr;
			break;
		}
	}
...
/*
UiGrid.CheckSelectedRow = function(aGrid, aSelectedList, aPositionOid2, aPositionClasz2) {
	UiGrid.ProcessSelectedRow(aGrid, aSelectedList, aPositionOid2, aPositionClasz2, function(aSelectedList, aSelectedObj, aCntrRow) {
		UiGrid.CheckAtRow(aGrid, aCntrRow, true); // checked the table row
	}, function(aSelectedList, aSelectedPosition, aCntrRow) {
		UiGrid.CheckAtRow(aGrid, aCntrRow, false); // checked the table row
	});
};
*/
UiGrid.AccumSelectedRow = function(aGrid, aSelectedList, aPositionOid2, aPositionClasz2) {
	UiGrid.ProcessSelectedRow(aGrid, aSelectedList, aPositionOid2, aPositionClasz2, function(aSelectedList, aSelectedObj) {
...
UiGrid.ProcessSelectedRow = function(aGrid, aSelectedList, aPositionOid2, aPositionClasz2, aOnAccum, aOnRemove, aOnNeedCheck) {
	if (UiUtil.IsUndefineOrNull(aGrid)) {
		return;
	}
	if (UiGrid.IsAllRowChecked(aGrid)) {
		aSelectedList.splice(0, aSelectedList.length);
		return;
	}
	...
	for(var cntrRow = 1; row = aGrid.table.rows[cntrRow]; cntrRow++) { 
		var aryInput = row.cells[UiGrid.PositionCheckBox].getElementsByTagName('input');
		if (aryInput.length > 0) { // if row is checked
			...
			if (aGrid.table._st_isNewTable) {
				...
				var listItemPosition = UiGrid.ExistInList(aSelectedList, JSON.stringify(selectedObj));
				...
				if (aryInput[0].checked) { // row checked
					if (UiGrid.NotInList(aSelectedList, JSON.stringify(selectedObj))) { // if not selected add to list
						aOnAccum(aSelectedList, selectedObj);
					}
