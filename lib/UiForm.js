...
UiForm.prototype.setupAryCtrl = function(cntrObj, newbtn, deletebtn, nxtbtn, strClsz, fieldsetName, slideId, slideNxt, slideSet, basePath, targetName) {
	...
	*/
	...
	if (UiUtil.IsUndefineOrNull(scriptElem)) {
		...
		//+ "sliderVar = UiUtil.CreateUpdateVerticalSlider(masterSlider, nextBtn);"
...
/*
UiForm.prototype.onNewFieldObjectChangeFieldClasz = function() {
	return(undefined);
};
*/
...
UiForm.prototype.newFieldObject = function(aFieldClasz, fieldsetTitle, basePath, targetName) {
	/*
	var fieldClasz = this.onNewFieldObjectChangeFieldClasz(aFieldClasz);
	if (UiUtil.IsUndefineOrNull(fieldClasz)) {
		fieldClasz = aFieldClasz;
	}
	*/
	...
	UiUtil.BeAction(requestParam, 'newField', this.baseUrl, function(jsonObject) {
		if (jsonObject !== null) {
			...
			if (targetField.dataset !== undefined) {
				...
				thisUiForm.enableDisableCancelButton();
			} else {
				jQuery.extend(true, targetField, jsonObject.dataset[0]);
				var avoidRecursive = new Array();
				avoidRecursive.push({clasz: thisUiForm.rawObj.meta.ParentClass, Oid: thisUiForm.rawObj.meta.ParentOid});
	...
	return function() {
		...
		if (targetField.dataset !== undefined) {
			...
			*/
			...
			/*
			var slideMaster = document.getElementById(slideId);
			var nextButton = document.getElementById(slideNxt);
			UiUtil.CreateUpdateVerticalSlider(slideMaster, nextButton);
			*/
			thisUiForm.setupAryCtrlButton(targetField, fieldsetTitle, slideId, slideNxt, basePath, targetName);

			/*
			// refresh / setup the array buttons for the slide
			...
			*/
			...
			//UiUtil.ClearAndPopulateSlideToDiv(fieldsetTitle, targetField, thisUiForm, setMaster, basePath, targetName);

			/*
			UiUtil.RemoveAllChildSlider(slideId);
			var masterSlider = document.getElementById(slideId);
			var cntrSlide = 0;
			for (var cntr = 0; cntr < targetField.dataset.length; cntr++) {
...
UiForm.prototype.saveObject2Backend = function() {
	UiUtil.DialogWaitStart();
	var editArea = this;
...
UiForm.prototype.cancelEdit = function() {
	UiUtil.PopulateJson(this.obj2Edit);
	if (UiForm.DifferentJson(this.objOriginal, this.obj2Edit)) {
		var thisForm = this;
		UiUtil.DialogOkCancel('Confirm to Cancel', 'You have made changes, confirm to cancel?', "question", function() {
			...
			//UiUtil.PopulateWidget(thisForm.obj2Edit);
...
UiForm.DifferentJson = function(aOriginal, aEdited) {
	...
	var strOriginal = jsonOriginal.match(/["]?data["]?\s?:\s?[^{](.+?)(?=[,}])/gm).toString();
	var strEdited = jsonEdited.match(/["]?data["]?\s?:\s?[^{](.+?)(?=[,}])/gm).toString();
	if (strOriginal !== strEdited) {
		result = true;
	} else {
		var strOriginalSet = jsonOriginal.match(/["]?dataset["]?\s?:\s?[^{](.+?)(?=[,}])/gm).toString();
		var strEditedSet = jsonEdited.match(/["]?dataset["]?\s?:\s?[^{](.+?)(?=[,}])/gm).toString();
